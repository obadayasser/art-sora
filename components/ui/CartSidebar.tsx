    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 bg-stage/60 backdrop-blur-sm z-50"
        >
          <motion.div
            initial={{ x: '100%' }}
            animate={{ x: 0 }}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {cartItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <button
                    onClick={() => setIsOpen(false)}
                    className="px-6 py-3 bg-[var(--color)] text-accent-contrast rounded-xl font-semibold hover:opacity-90 transition-opacity"
                  >
                ...
                <div className="space-y-4">
                  {cartItems.map((item) => {
                    const itemName = locale === 'ar' ? item.product.nameAr : item.product.nameEn;
                    return (
                      <motion.div
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-[var(--foreground)]/60 mb-2">
                            {t('cart.sku')}: {item.product.sku}
                          </p>
                          <div className="flex items-center gap-3 mb-2">
                            <button
                              onClick={() => updateQuantity(item.product.id, item.quantity - 1, item.sizeId)}
                              aria-label="Decrease quantity"
                              className="w-8 h-8 flex items-center justify-center rounded-lg bg-[var(--card-border)] hover:bg-[var(--color)] hover:text-accent-contrast transition-colors"
                            >
                          ...
                          <div className="flex items-center justify-between">
                            <p className="font-bold text-[var(--color)]">
                              {t('product.currency')} {((parseFloat(item.product.salePrice || item.product.basePrice) * item.quantity).toFixed(2))}
                            </p>
                            <button
                              onClick={() => removeFromCart(item.product.id, item.sizeId)}
                              aria-label="Remove item"
                              className="p-2 rounded-lg hover:bg-danger/10 hover:text-danger transition-colors text-[var(--foreground)]/60"
                            >
              ...
              <div className="p-6 border-t border-[var(--card-border)] bg-[var(--background)]">
                <div className="space-y-3 mb-6">
                  <div className="flex justify-between text-[var(--foreground)]/80">
                    ...
                    <span>{t('product.currency')} {getCartTotal().toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-[var(--foreground)]/80">
                  ...
                  <div className="flex justify-between text-lg font-bold text-[var(--foreground)] pt-3 border-t border-[var(--card-border)]">
                    ...
                    <span className="text-[var(--color)]">{t('product.currency')} {getCartTotal().toFixed(2)}</span>
                  </div>
                </div>
                <Link
                  href="/checkout"
                  onClick={() => setIsOpen(false)}
                  className="block w-full py-4 bg-accent text-accent-contrast rounded-xl font-bold text-center hover:bg-accent-hover transition-colors shadow-lg"
                >
                  {t('cart.checkout')}
                </Link>
                <button
